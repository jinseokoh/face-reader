...
## 3. 우분투에서 Docker 없이 직접 실행 (개발용) _(docker 환경에서 불필요)_

> **이 섹션은 docker compose 로 실행할 거면 건너뛰어도 된다.** 로컬에서 코드 자주 고치며 확인하거나, docker 가 없는 환경에서 돌릴 때만 참고.

Docker 가 부담스럽거나 코드를 자주 고치면서 확인하고 싶을 때.

### 3-1. 시스템 패키지 설치 _(docker 환경에서 불필요)_
...
### 3-2. 가상환경 만들기 _(docker 환경에서 불필요)_
...
### 3-3. 의존 라이브러리 설치 _(docker 환경에서 불필요)_
...
### "libGL.so.1: cannot open shared object file" _(docker 환경에서 불필요)_
OpenCV 가 의존하는 시스템 라이브러리가 없는 경우. Docker 를 안 쓸 때 발생한다:
```bash
sudo apt-get install -y libgl1 libglib2.0-0 libsm6 libxext6 libxrender1
```

### "ModuleNotFoundError: No module named 'fastapi'" 등 _(docker 환경에서 불필요)_
...
### "You have tensorflow ... and this requires tf-keras package" _(docker 환경에서 불필요)_
