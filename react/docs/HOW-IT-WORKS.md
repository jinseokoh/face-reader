...
- Supabase Postgres + REST — `metrics` 테이블 (UUID PK, `metrics_json` JSONB, `views` int, `updated_at` timestamptz). 행 자체는 anonymous (user_id 없음).
...
Flutter
  ...
  └─ Supabase REST UPSERT /rest/v1/metrics             → Supabase (anon key)
        body: { id: "<uuid>", metrics_json: { … thumbnailKey 포함 … } }
...
비용: thumbnail 10 KB × 100만 카드 ≈ R2 $0.15/월. metrics_json 2 KB × 100만 행 ≈ Supabase 2 GB. inactive cleanup 으로 자연 감소.
...
```jsonc
{
  ...
  "race": "eastAsian",           // app Ethnicity enum ↔ deepfaceRace

  "deepfaceAge": 28,             // DeepFace raw int (정제값은 ageGroup decade 라벨)
  "deepfaceGender": "Man",       // DeepFace raw "Man" | "Woman"
  "deepfaceRace": "asian",       // DeepFace raw — 매핑 안 함
...
| metrics_json (rawValue·demographic 카테고리·thumbnailKey·deepface raw·schemaVersion·source·timestamp·faceShape) | 비-PII | Supabase `metrics` | 정규화된 값만. landmark 좌표·이름·생년월일 0. |
...
```sql
-- Supabase pg_cron, 매일 03:00 KST
delete from metrics where updated_at < now() - interval '3 months'
returning id;
```

returning 으로 받은 uuid 리스트를 Cloudflare Worker Cron Trigger 가 매일 같은 시각에 받아 R2 thumbnail 도 같이 DELETE. 또는 Worker 가 직접 Supabase 의 `select id from metrics where updated_at < now() - interval '3 months'` 로 list 받고 R2 DELETE 후 마지막에 SQL delete.

순서는 R2 DELETE 먼저 → Supabase 행 DELETE 가 안전 (R2 실패 시 행 살아있어 재시도 가능).
