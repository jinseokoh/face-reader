...
## 3. Schema — Flutter 의 `metrics` 테이블 그대로 + 공유 엔진
...
`share_payload` jsonb shape (Flutter 가 metrics row 저장 시점에 함께 채움):

```json
{
  "score": 78,                                    // 0~100 종합 점수 (archetype.totalScore)
  "archetype": "온화한 학자형",                     // archetype label
  "five_element": "earth",                        // wood|fire|earth|metal|water
  "short_summary": "균형 잡힌 흐름이 보이는 얼굴.", // 1줄, no PII
  "highlights": [                                  // top 3 corePoints
    {"title": "面相 — 균형", "detail": "이마·코·턱 비율이 안정적"},
    {"title": "氣 — 温和", "detail": "기운이 모나지 않고 고르게 퍼짐"},
    {"title": "性情 — 솔직", "detail": "꾸밈 없이 직진하는 성정"}
  ]
}
```

**share_payload 에 절대 넣지 말 것**:
- 친밀(intimacy) 챕터 본문
- 갈등 시나리오 상세 텍스트
- 사용자 이름·생년월일·얼굴 이미지
- raw landmark · z-score map · rule evidence
- archetype 외 personalized 본문

`metrics_json` 의 어떤 부분도 그대로 옮겨 담지 말 것 — share_payload 는 anon 이 view 통해 읽으므로 rendering 에 필요한 minimal subset 만.

view 의 `expires_at > now()` 필터로 만료 row 는 자동 제외 → React loader 에서 `length !== ids.length` 면 404.
...
`flutter/lib/domain/services/` 의 다음 파일을 `/shared/lib/` 로 이동:
- `physiognomy_scoring.dart` (14-node tree)
- `attribute_derivation.dart` (5-stage pipeline)
- `attribute_normalize.dart` (성별 quantile)
- `score_calibration.dart`
- `archetype.dart` (top-2 + special)
- `face_metrics.dart` + `face_metrics_lateral.dart` (아니면 raw 만 받고 metric 계산은 Flutter 가 했으니 필요 시만)

`flutter/lib/data/constants/face_reference_data.dart` 도 함께 이동. Flutter 의 `pubspec.yaml` `dependencies:` 에 `shared: { path: ../shared }` 추가, import 경로를 `package:shared/...` 로 일괄 갱신.

`/shared/lib/face_engine.dart` 는 단일 진입점 — `runEngine(String metricsJson) → Map` (score, archetype, highlights[3] 만 노출). 내부에서 5-stage pipeline + classifyArchetype + 룰 적용까지 다 끝내고 minimal subset 만 return.

빌드는 `react/package.json` 의 `build:shared` 스크립트 (dart compile js).
...
- [ ] Supabase 마이그레이션 — `metrics.share_payload` 컬럼 추가, `revoke select on metrics from anon` + `grant select (id, share_payload) on metrics to anon` + RLS policy `share_payload not null and expires_at > now()`
- [ ] Flutter `SupabaseService.saveMetrics`/`upsertMetricsJson` 가 `share_payload` 도 같이 저장
- [ ] share_payload 에 친밀 챕터·갈등 본문·이름·생년월일·얼굴 이미지·raw landmark 안 들어감 (코드 review 필수)
