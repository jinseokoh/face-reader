...
> ⚠️ **후속 문서 있음 — `PLAN-THUMBNAIL-REGISTRY.md`.** 이 계획의 회수(D4)는
> 참조를 `metrics` 에서만 센다. 결제된 궁합 스냅샷도 같은 키를 붙들고 있어
> 실제 데이터 손실이 났다. cron 배포와 1회 정리 스크립트 재실행을 보류 중이다.
...
| P3 아웃박스 | ⚠️ 코드 완료·마이그레이션 적용됨. **배포 보류** — 참조 계수가 반쪽이라 배포하면 결제된 궁합의 사진을 지운다 (`PLAN-THUMBNAIL-REGISTRY.md`) |
| P3-4 1회 정리 | 🔴 실행함 — 객체 98개 중 28개 삭제 (2026-08-22). **그중 일부는 고아가 아니었다.** 스크립트가 `metrics` 만 참조로 세어, 결제된 궁합 스냅샷이 붙들고 있던 썸네일 7개를 지웠다. **재실행 금지** |
...
1. **공유받은 카드**: 원 소유자가 카드를 지우면 아웃박스가 객체를 회수하고,
   ...
   → **결제된 궁합은 반대다.** `privacy.md:27` 이 "상대방의 데이터 삭제 여부와
   무관하게 이용" 을 명시한다. 두 경우를 같은 축에서 다시 봐야 한다
   (`PLAN-THUMBNAIL-REGISTRY.md` §4).
