...
- **안면 계측값**: 이용자 기기 안에서 얼굴 랜드마크(468점)를 검출한 뒤 계산한 비율·각도·좌우 대칭 정도 등의 수치. 랜드마크 좌표 자체는 저장하지 않습니다.
...
- 2026-09-06 — 얼굴 데이터 조항에 첫인상 지표(계측값에서 계산한 파생값) 항목·목적·보유 기간을 추가하고, 프로파일링이 아님을 명시. 안면 계측값에 좌우 대칭 정도를 포함
