...
+ 내 관상 확인(+ 비밀방이면 PIN, 공약이 있으면 동의)을 마치는 순간 셀프 조인된다.
...
- **방장**: 배틀 생성(방 이름·인원·공개/비밀·연령대·공약) → 로비 진입 → QR 코드·
...
- **합류자 (앱 설치)**: 링크 탭 → 유니버설/앱 링크로 앱 직행 → `BattleJoinScreen`
  이 방 정보(n/N명·연령대·공약) 미리보기 표시 → 로그인 → 내 관상 확인(없으면 촬영)
...
- **앱 미설치 (웹 참여)**: 브라우저로 `/g/{id}` — 모집 중 = 초대장 + **웹 참여
  ...
  (공약) 동의 → 정면 캡처 → metrics+R2 썸네일 저장 → `join_battle` RPC 로 셀프
...
- 서버: `0001_baseline.sql` §11-2~11-5 적용(teams·team_members + RLS + RPC 상태
  머신 + view + Realtime), AASA·assetlinks 에 `/g/*`.
...
| `chat_url` 수동 입력 | 카카오 오픈링크(오픈채팅) 자동 발행 API 는 도메인 ID 발급이 제휴 게이트라, 방장이 오픈채팅을 직접 만들어 링크만 붙여넣는다 |
...
| DB 스키마 + RPC 상태 머신 | `react/db/migrations/0001_baseline.sql` §11-2~11-5 |
