...
| 명단 데이터 | **current my-face live resolve** 유지 (기존 패턴 그대로). 유일한 동결 = `result_payload` (경기 기록) |
...
**"시작 수행"** = `status='revealing'` + `started_at=now()` — 그게 전부다.
동결 UPDATE 없음. 얼굴 데이터는 reveal 계산 시점에 참가자 `user_id` 들의 현재 my-face 를
한 쿼리로 live resolve 하고, 유일한 동결은 `submit_battle_result` 가 쓰는 payload 다.
...
status='revealing' broadcast 수신 → 각 클라이언트가 참가자 user_id 들의 현재 my-face body 를
한 쿼리로 fetch (`metrics where user_id in (…) and is_my_face`) → `runBattle` 로컬 계산 →
3-2-1 카운트다운 연출 → 어워드 순차 flip.
최초 도달 클라이언트가 `submit_battle_result` 로 payload backfill — **이 payload 가 유일한 동결**
(결정론 + first-writer-wins 라 reveal 직전 재촬영 같은 미세 race 도 payload 가 정본으로 수렴).
...
- **참가 중 my-face 재촬영**: 언제든 무해 — payload 기록 전엔 live resolve 가 최신을 반영하고, 기록 후엔 payload 가 정본.
