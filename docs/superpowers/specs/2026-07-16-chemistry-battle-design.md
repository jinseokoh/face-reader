...
**동점 처리 (전 어워드 공통)**: 순위 비교는 표시용 반올림 점수가 아니라 엔진 raw total(double)로.
raw 까지 완전 동점이면 결정론 tie-break — 쌍 어워드는 `(min slot_no, max slot_no)` 사전순 우선,
Leader 는 낮은 slot_no 우선. **공동 수상 없음** (단독 수상이 연출·공약 회수를 단순하게 유지).
전 클라이언트가 같은 snapshot 에 같은 규칙을 적용하므로 payload 수렴이 유지된다.
...
공약 회수 카드는 컬럼 + `awards.best` 조합으로 렌더.
...
3. 2기기 시나리오: A 생성(공약 포함) → B 웹 조인 → 정원 충족 자동 시작 → 양쪽 동시 카운트다운 → 같은 어워드 → payload backfill 1회 → `/g/:id` 쇼케이스.
