...
create table team_members (
  ...
  user_id     uuid not null references auth.users(id) on delete cascade,
                 -- 계정 삭제 = 참가 행 소멸 → 슬롯 자동 반환 (recruiting 중 resolve 불능 참가자가 구조적으로 없음)
...
resolve 실패는 구조적으로 없음 — 조인 가드가 my-face 를 요구하고, my-face 는 강등만 될 뿐
삭제되지 않으며(재촬영·claim 모두 유지), 계정 삭제는 FK cascade 로 참가 행 자체를 지운다.
