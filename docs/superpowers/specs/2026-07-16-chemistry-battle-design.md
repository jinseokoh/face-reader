...
| 명단 데이터 | 시작 시점 **동결** (배틀 = 그 순간의 경기 기록; my-face 최신성 모델 자체는 불변) |
...
create table teams (
  ...
  max_players    int  not null default 8,           -- 4~12
  min_players    int  not null default 4,           -- 3~max
...
create table team_members (
  ...
  metrics_id  uuid references metrics(id) on delete set null,
                 -- recruiting 중 = 참가 시점 my-face. 시작 시 live resolve 로 동결
...
| `start_battle(team_id)` | caller = owner · status='recruiting' · 인원 ≥ min_players | 시작 수행 |
...
**"시작 수행"** = 각 참가자의 live my-face(`metrics where user_id=? and is_my_face` 최신)를
`metrics_id` 에 UPDATE 로 동결 + `status='revealing'` + `started_at=now()`.
resolve 실패(로그인했지만 my-face 삭제 등) 참가자는 조인 가드가 원천 차단하므로
정상 경로에선 발생하지 않고, 발생 시 해당 참가자 제외 후 인원 ≥ min 이면 진행,
미달이면 expired 처리.
...
recruiting ─ 정원 충족(join_battle 내장) ────────────► revealing
           ─ 방장 [지금 시작] (≥ min) ───────────────► revealing
           ─ cron: 48h 경과 & ≥ min ────────────────► revealing (서버측 동결)
           ─ cron: 48h 경과 & < min ────────────────► expired
...
- `react/workers/cron.ts` 확장: closeStaleTeams → 위 분기(시작 or expired)로 교체.
  cron 의 시작 수행은 service-role 로 동일 동결 로직.
- recruiting 중 참가자 나가기 허용(leave_battle), 방장 방 삭제 허용. 시작 후 명단 동결.
...
status='revealing' broadcast 수신 → 각 클라이언트가 동결된 metrics_id 들의 body fetch →
`runBattle` 로컬 계산 → 3-2-1 카운트다운 연출 → 어워드 순차 flip.
최초 도달 클라이언트가 `submit_battle_result` 로 payload backfill (결정론이라 first-writer-wins 무해).
...
| 방 생성 (`team_create_page` 개편) | 스텝 플로우 재활용: 방 이름 → 인원(최대 4~12/최소) → 공개/비밀(+PIN 4자리) → 연령대 → 내기(선택: 유형+프리셋+오픈채팅 링크) → 생성 즉시 로비. 이름 칩 스텝 삭제 |
| 로비 (`team_lobby_screen` 신설, TeamRoomScreen 대체) | n/N 슬롯 그리드(입장 실시간 반영: 아바타+닉네임 flip-in) · 판돈 배너 · **QR 코드**(현장 셀프 조인 — walk-in 의 계승) · 초대 3버튼 유지 · 방장 [지금 시작](≥min 활성) · 참가자 [나가기] |
...
- **참가 중 my-face 재촬영**: recruiting 중엔 영향 없음(시작 시 live resolve 로 최신 반영), 시작 후엔 동결값 사용.
- **방장 이탈**: 방장은 leave 불가(방 삭제만). 방장 탈퇴(계정) 시 owner_id set null — cron 수명주기가 정리.
- **연령대 미달 조인 시도**: RPC 가 명시 에러 코드 반환 → "이 방은 20~39세 참가 방입니다" 안내.
- **min 미달 expired**: 웹·앱 모두 "인원이 모이지 않아 종료" — 기존 카피 유지.

---

## 10. 검증 계획

1. RPC 단위: SQL 테스트(정원·중복·PIN·연령·상태 가드 각각) — Supabase 콘솔 스크립트.
...
4. 현장 시나리오: 로비 QR → 3인 셀프 조인 → 방장 수동 시작.
5. cron: 48h 시작/만료 분기, 24h revealing 안전망.
