...
> 전 쌍 케미 분석 → 게임 결과처럼 공개한다 (🏆 Best 쌍 + 밴드 매트릭스).
...
| 공약 | **방장이 방 생성 시 설정** — 방의 정체성. "베스트 케미 둘이 하는 것" 단일 개념 |
| 과금 | 불변 — Best 카드·밴드 매트릭스 무료, 쌍 상세 1🪙 unlock |
...
create table teams (
  ...
  pledge         text,          -- 공약: 베스트 케미 둘이 하는 것. 프리셋 라벨 또는 직접입력 (40자 캡). null = 없음
  chat_url       text,          -- 카카오 오픈채팅 링크 (선택, 당첨 쌍 연결 고리)
...
- 삭제: `matrix_payload` → `result_payload` 로 대체 (best 포함, §6.3).
...
**공약 공개방 성인 게이트**: `visibility='public' and pledge is not null` 이면
...
- 공개 목록: `visibility='public' and status='recruiting'` 조회 + 참가자 수 count 를 담은 **view `public_battles`** (컬럼 화이트리스트: id·title·인원·연령대·pledge·created_at, `security_invoker=on` + write revoke — auto-updatable view 의 RLS 우회 쓰기 차단).
- 명단 조회: **view `battle_roster`** (team_id·user_id·slot_no·is_owner·joined_at·**nickname**) — users RLS 가 self-read 라 참가자 닉네임을 읽을 유일한 경로. owner 권한 실행으로 nickname 만 노출 (coins 등 비노출), 로비·리빌·payload 조립이 전부 이 view 를 쓴다.

### 2.5 Realtime

baseline 에 포함 — `team_members` 는 전체, `teams` 는 **컬럼 리스트 발행**(password·chemistry_snapshot·result_payload 제외, PG15) — Realtime full-row payload 는 column grant 를 우회하므로 발행 단에서 봉인. 이벤트는 신호이고 본문은 refetch.
...
개념은 하나: **베스트 케미로 뽑힌 두 사람이 공약을 실행한다.**
("쏜다"류 정산 개념 없음 — 현실 강제력이 없어 무용지물, 2026-07-16 폐기)

- 프리셋: 🎬 영화 · ☕ 커피 · 🍜 밥 한 끼 · 🎤 노래방 · 직접입력 (40자).
- 비밀방(친목) = 아는 사이의 미션, 공개방 = stranger 매칭 훅 ("영화보러가자!" 방).
- 방 목록 카드·초대장·로비 배너에 공약 상시 노출 — 입장 전에 걸린 것이 보인다 (참가 훅).
- 회수: 🏆 Best 카드 직후 공약 카드 flip — "이 방의 공약: 🎬 영화는 ○○·○○님이".
  당첨 쌍에게만 `chat_url`(오픈채팅) 버튼 노출.
...
- 자유텍스트(pledge·title)는 신고·차단 backlog(PRD §6.2-3)와 같은 모더레이션 표면.

---

## 5. 결과 = 🏆 Best 쌍 + 밴드 매트릭스

결과물은 딱 둘이다 (Rival/Leader/Unexpected/Hidden 류 파생 시상 없음 — 2026-07-16 폐기):

1. **🏆 Best Chemistry** — raw total 최고점 쌍. 점수 공개 무료 (도파민 모먼트). 직후 공약 카드 회수.
2. **N×N 밴드 매트릭스** — 셀은 밴드 이모지만 (점수 없음), 쌍 상세 해석 = 1🪙 unlock (기존 `unlock_compat` 그대로).

**동점 처리**: Best 비교는 표시용 반올림 점수가 아니라 엔진 raw total(double)로.
raw 까지 완전 동점이면 결정론 tie-break — `(min slot_no, max slot_no)` 사전순 우선.
**공동 수상 없음** (단독 수상이 연출·공약 회수를 단순하게 유지). 전 클라이언트가 같은
snapshot 에 같은 규칙을 적용하므로 payload 수렴이 유지된다.
(기존 `team_matrix.dart` 의 "반올림 점수 동점 전원 포함" 방식은 승계하지 않는다.)
...
`shared/lib/domain/services/compat/` 아래로 이동 — 산출은 쌍별 밴드 + best(동점 tie-break 포함).
JS export `runBattle(bodiesJson)` 신설 (기존 runEngine/runCompat/runMetrics 와 나란히) —
앱과 웹이 단일 코드로 같은 결과를 계산한다. React 쪽 재구현 금지 원칙 유지.

### 6.2 계산·공개 흐름

status='revealing' broadcast 수신 → 각 클라이언트가 `teams.chemistry_snapshot` 한 컬럼 fetch →
`runBattle(snapshot)` 로컬 계산 → 3-2-1 카운트다운 연출 → 🏆 Best 카드 공개.
...
| 방 생성 (`team_create_page` 개편) | 스텝 플로우 재활용: 방 이름 → 인원(최대 4~12) → 공개/비밀(+PIN 4자리) → 연령대 → 공약(선택: 프리셋+오픈채팅 링크) → 생성 즉시 로비. 이름 칩 스텝 삭제 |
...
| 결과 연출 (`team_reveal_screen` 신설) | 3-2-1 카운트다운 → 🏆 Best 카드 flip → 공약 회수 카드(있을 때) → N×N 밴드 매트릭스(보는 사람 행 최상단 고정 유지) → 쌍 탭 = 1🪙 unlock |
...
| `/g/:id` completed | 쇼케이스: result_payload 의 🏆 Best 카드 + 밴드 매트릭스 (사진 없음·점수는 Best 만 — 기존 프라이버시 정책 유지) |
...
2. `shared/` runBattle: Dart 테스트 — 밴드·best 결정론 + 동점 tie-break + 기존 151 green 유지. `pnpm build:shared` 통과 (-O1).
3. 2기기 시나리오: A 생성(공약 포함) → B 웹 조인 → 정원 충족 자동 시작 → 양쪽 동시 카운트다운 → 같은 best → payload backfill 1회 → `/g/:id` 쇼케이스.
