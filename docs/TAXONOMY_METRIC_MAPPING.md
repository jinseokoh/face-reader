...
3. **Rule**: `attribute_engine.dart` 의 규칙에서 참조
4. **UI**: 리포트 화면에 노출
...
3. **규칙 시스템 전환 수준**: **(a) 전면 재구조화** — `attribute_engine.dart` 720 LOC
   를 node-scoped 체계로 재작성. Phase 3 에서 실행.
...
- 규칙 밀도(과집중) 갱신은 `attribute_engine.dart` 변경 커밋에서 동시 진행
