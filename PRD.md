...
**상태 스냅샷**: 케미 그룹 rev2(방 유형·매칭 성사·인앱 채팅) 서버·웹·앱 전면 배포 완료 · `flutter test` 168 green · 코인 IAP(coin_3/coin_14) 스토어 등록 · Android Play `1.0.3+15` 재제출 준비(2026-07-30 — 비밀방 capability 모델·채팅방 나가기·매칭 48h 만료·강제 업그레이드 app_config·설정 배너 팝업·UI 전면 다듬기) · iOS 재제출 대기
...
용어 규칙: 한국어 공식 명칭 = **`케미 그룹`**(2026-07-19 "케미 매칭"에서 재개칭, `케미` 단독 표기는 2026-07-16 폐기). 방/모임 단위는 `그룹`(공개 그룹·내 그룹 — 단, 케미 탭의 첫 탭 표기는 `모집중`, 2026-07-30 재명명), 성사된 쌍은 `매칭`. 코드 식별자 = 영문 `team_*`. 시간 표기는 hero 메타 줄의 "yyyy.mm.dd ↔ n시간 후 마감" 문법(2026-07-30 — 2026-07-12 의 "발표/마감 폐기" 방침을 대체).
...
- [ ] 웹 JoinWizard 의 비밀방 PIN 스텝 제거 — capability 모델(2026-07-30)로 `join_team` 이 비밀번호를 더는 검사하지 않아 형식적 절차가 됨
...
| 페르소나            | 상황                                          | 행동                                                                                                        |
| ------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **방장(호스트)**    | 회식·MT·동아리 모임에서 분위기를 만드는 사람  | 매칭 방 생성(방 유형·제목 프리셋·인원·연령대·공개/비밀·썸네일 공개 설정) → 상세 페이지에서 QR·카톡·링크로 초대 → 정원 충족 시 자동 결과 공개 |
| **참가자(앱 설치)** | 상세 페이지 QR 을 스캔했거나 링크를 받은 사람 | 로그인 → 내 관상 확인(없으면 촬영) → 사진 공개 계약 확인 → 셀프 조인 → 상세 페이지 실시간 슬롯 확인 (비밀방 PIN 은 앱 내 목록 진입 시에만 — 초대 링크 소지 = 초대받음, capability 모델 2026-07-30) |
...
| 앱 미사용자              | React 웹(`/g/{id}`)에서 **셀프 조인 완결** — 카카오 로그인 → 사진 공개 계약 → 정면 캡처 → `join_team` (PIN 스텝은 UI 에 남아 있으나 서버가 검사하지 않음 — 제거 TO-DO)  | ✅ 구현 (`JoinWizard`)                                             |
...
- 홈/케미 탭 [매칭 만들기] → **바텀 슬라이드 풀페이지 스텝 플로우**:
  ...
  PIN + 썸네일 공개 여부) → [매칭 만들기] = 방 생성 + 방장 셀프 조인.
...
- 헤더 = 다크 그라데이션 hero(`TeamStatHeader`, 관상·궁합 상세와 동일 문법):
  좌상단 "yyyy.mm.dd ↔ n시간 후 마감" 메타 줄, '케미도 과학이다' 라벨 + 송명체
  제목(2줄) + 그린 연령·정원("8명 그룹") chip, 반투명 구분선 아래 **참가자
  그리드가 hero 안에** 들어간다 (참가자 뷰는 발표 조건 반투명 컨테이너 병기).
- 참가자 그리드: 아바타 좌 + "50대 남성 아시아인" 인구통계·"신의형 · 호감형 기질"
  관상 유형 meta 우(흰색·sand 톤), 내 행은 아바타 좌상단 '나' badge.
  ...
  **모집 중에 한해** 아바타 대신 성별 실루엣 svg — 결과 발표
  후에는 참가자 전원 사진 공개가 조인 계약이다 (`thumb_open` 은 모집 중 노출만
  제어).
- **QR 코드**('이 그룹으로의 초대' 섹션): 같은 자리 참가자가 스캔해 셀프 조인 —
  현장 참여의 유일한 경로다. 초대 3버튼(카톡/링크 공유/복사) 상시 공존 (참가자 뷰).
- 참가자는 [나가기] 가능(방장은 방 삭제만). 정원 충족 시 시작 버튼 없이 자동 전환된다.
- 미참가자 뷰(딥링크 `/g/:id` 진입 포함 — 링크 진입은 `receivedView` AppBar 닫기 X):
  방 정보 미리보기 → 로그인 → 내 관상 보장(없으면 촬영) → 사진 공개 계약 확인 →
  `join_team` RPC 로 셀프 조인. **PIN 인풋 없음** — 비밀방 문턱은 앱 내 모집중
  목록의 문 앞 dialog(`check_team_password`) 하나뿐 (capability 모델: teamId 가
  추측 불가능한 UUID 라 초대 링크 소지 = 초대받음, `join_team` 도 검사 안 함).
- 정체성은 로그인 계정(`users.nickname`) 하나뿐 — 이름 슬롯·빈 슬롯 claim 은 없다.

#### 결과 화면

- 3-2-1 카운트다운 → 베스트 케미(hero 다크 박스 안 — 인물 2열 + × glyph + 등급
  성어 클라이맥스, raw total 최고점 쌍, 점수 무료 공개) → 케미 매트릭스(`all` 방 =
  N×N, `match` 방 = 남×여 직사각 — 이성 쌍만 계산·저장·표시. 셀 =
  ...
  **보는 사람 행 최상단 고정**) → 나와의 케미 순위 → 쌍 탭 = 1🪙
  unlock(`unlock_compat` RPC).
...
- 베스트 쌍 두 사람에게만 결과 직후 매칭 성사 카드 — 서로의 200×200 사진 공개(성사
  ...
  열까요?" 상호 동의(`respond_match` RPC, 수락/거절 1회 — 재응답 불가. **응답 시한 =
  발표 후 48h** — 경과하면 자동 거부 취급: 서버 MATCH_EXPIRED + 카드 버튼 숨김).
- 둘 다 수락 → 인앱 1:1 채팅(`team_messages`, Realtime) — 내 그룹 탭에 [채팅] 진입점.
  한쪽 거절 → 즉시 종결("이번에는 채팅방이 열리지 않았습니다"). 무응답 → 48h 만료 후
  30일 purge 와 함께 소멸. 채팅 수명 = teams 30일 purge cascade. 채팅방은
  `leave_chat` 으로 나갈 수 있다(⋮ 메뉴 — 본인 목록에서만 숨김·재진입 불가, 상대는
  대화 유지 + 카톡식 나감 시스템 라인, 입력바는 유지).
- 베스트 쌍이 아닌 참가자에겐 이 단계 자체가 없다.
- **매칭 응답 푸시(FCM)**: 상대의 수락/거절은 앱 밖에서 일어나는 사건이라 푸시로
  알린다. `team_matches` consent 변경 trigger → Worker `/api/push/match` →
  FCM v1. 문구 3종(사실만): 둘 다 수락 "채팅방이 열렸습니다" / 상대 선수락
  "{닉}님이 채팅을 수락했습니다" / 거절 "이번에는 채팅방이 열리지 않았습니다".
  탭하면 `/g/{id}` 딥링크(상세 → 리빌·매칭 카드 자동 연결). 포그라운드에서도
  같은 시스템 알림을 로컬로 띄운다(앞뒤 동일 UX). 기기 token 은
  `push_tokens`(로그인 시 upsert, 로그아웃 시 삭제). Android 우선 — iOS 는
  APNs 설정 후 동일 코드로 동작.
- **채팅 메시지 푸시**: `team_messages` INSERT trigger → `/api/push/chat` →
  상대 기기. 알림 = 보낸이 닉네임 + 본문 미리보기(80자), 탭하면 채팅방
  직행(`/chat/{id}`). 그 채팅방을 보고 있는 중엔 배너 생략(Realtime 이 즉시
  그림). 채널 분리(매칭 알림/채팅 알림) — 사용자가 각각 끌 수 있다.
...
- 제3자 방에서 같이 매칭하게 되면 그 쌍의 발표 점수를 엔진이 **상한 60점**(형극난조
...
- 유료 풀이(1🪙)는 실제 관상 분석 그대로 — 상한은 매칭 결과표에만 적용된다(같은 두
...
- **모집 중 = 초대장 + 웹 참여 위저드(JoinWizard)**: 방 이름·"n/N명"·연령대(match 방은
  ...
  같은 `auth.users`) → 사진 공개 계약 → 정면 캡처(MediaPipe client-only)
  ((비밀방) PIN 스텝은 UI 에 남아 있으나 서버 미검사 — 제거 TO-DO)
