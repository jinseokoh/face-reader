...
| 우선 | 작업 | 근거 | 재개 지시 |
|---|---|---|---|
| **P0** | **카카오 공유 — share_publisher 구현** | AppBar 우상단 `FA.commentDots` 한 버튼. 공유 클릭 시 R2 thumbnails/ 256 JPG PUT (Worker presign) + Supabase `metrics.body` UPDATE (`thumbnailPath` 제거·`thumbnailKey` 추가·`expires_at` null) + `kakao_flutter_sdk_share` Feed template 호출 (title·description·image·`자세히 보기` 버튼). **Hybrid thumbnail**: Hive 는 `thumbnailPath`(local) 영구 유지 + publish 후 `thumbnailKey`(R2) 추가. 표시 우선순위 local→R2 fallback. **Supabase body 에는 `thumbnailKey` 만** (PII: 디바이스 path 제외). 궁합 공유 = 양쪽 metrics 가 publish 된 상태에서 `/r/{A}~{B}` 발송 (write 0). | `Kakao Developers 콘솔 앱 등록 + App Key 발급 → ios/Runner/Info.plist (LSApplicationQueriesSchemes: kakaolink, kakao{KEY}) + AndroidManifest intent-filter → main.dart KakaoSdk.init(nativeAppKey). lib/domain/services/share/share_publisher.dart 작성: publishSolo/publishCompat 두 함수. FaceReadingReport 에 thumbnailKey 슬롯 신규. main.dart 에 app_links + /r/:id 라우팅.` |
