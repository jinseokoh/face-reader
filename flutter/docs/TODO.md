...
| 우선 | 작업 | 근거 | 재개 지시 |
|---|---|---|---|
| **P0** | **공유 link 통합 — share_publisher 구현** | AppBar 2 button (`FA.kakao` + `FA.squareInstagram`) 우상단. 공유 클릭 시 R2 thumbnails/ 256 JPG PUT (Worker presign) + Supabase `metrics.body` UPDATE (`thumbnailPath` 제거·`thumbnailKey` 추가·`expires_at` null). 카톡=KakaoLink Feed template, IG=`social_share.shareInstagramStory(image, attributionURL)`. **Hybrid thumbnail**: Hive 는 `thumbnailPath`(local) 영구 유지 + publish 후 `thumbnailKey`(R2) 추가. 표시 우선순위 local→R2 fallback. **Supabase body 에는 `thumbnailKey` 만** (PII: 디바이스 path 제외). 궁합 공유 = 양쪽 metrics 가 publish 된 상태에서 `/r/{A}~{B}` 발송 (write 0). | `react/docs/HOW-IT-WORKS.md §3.2/§4.1/§5.2 따라 lib/domain/services/share/share_publisher.dart 작성. publishSolo / publishCompat 두 함수. main.dart 에 app_links + KakaoSDK 초기화 + /r/:id 라우팅 (PAIR_SEP("~") split → 1 UUID → ReportPage, 2 → CompatReportPage). FaceReadingReport 에 thumbnailKey 슬롯 신규.` |
