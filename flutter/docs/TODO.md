...
#### RevenueCat 실 스토어 전환 체크리스트

현재 RevenueCat 의 **Test Store** (key prefix `test_`) 로 mock IAP 만 동작. 실제 결제 받기 위해선 아래 모두:

**1) App Store / Play Console 상품 등록**
- [ ] App Store Connect → My Apps → Facely → In-App Purchases — `coin_3`, `coin_10` 두 개 Consumable 추가 + Reference Name·Display Name·Price·Review note 작성 → "Ready to Submit" 상태로
- [ ] Play Console → Facely → Monetize → Products → In-app products — 동일 ID 두 개 추가 → "Active"
- product ID 는 코드 SSOT (`flutter/lib/data/services/coin_service.dart:29` `_productIds`) 와 일치해야 함. 변경 시 코드 + 두 console 동시 수정

**2) RevenueCat 앱·키 발급**
- [ ] RevenueCat dashboard → Project "Facely" → Apps → "+ New" → App Store 앱 추가 → Bundle ID `com.scienceintegration.facely` + App-Specific Shared Secret (ASC 에서 발급)
- [ ] 동일하게 Google Play 앱 추가 → package name + service account JSON 업로드
- [ ] 각 앱의 **Public app-specific API key** 복사 (`appl_…`, `goog_…`)

**3) RevenueCat Product / Offering 매핑**
- [ ] Product catalog → Products → `coin_3`, `coin_10` 를 위 두 store 에 연결 (Test Store 항목은 그대로 둬도 됨)
- [ ] Offerings → "default" Offering 에 두 product 를 Package 로 배치 (offering 안 쓰면 생략 가능 — 현재 코드는 `Purchases.getProducts(_productIds)` 직접 fetch 이므로 무관)

**4) Flutter `.env` 키 교체**
```
REVENUECAT_API_KEY_IOS=appl_xxxxxxxxxxxxxxxxxxxxxxxxx
REVENUECAT_API_KEY_ANDROID=goog_xxxxxxxxxxxxxxxxxxxxxxxxx
# REVENUECAT_API_KEY=test_...    ← 삭제 (Test Store 키)
```

**5) 결제 흐름 sandbox 검증**
- [ ] iOS: TestFlight 빌드 + Sandbox tester 계정으로 결제 → coins 증가 + `addCoins` RPC 호출 확인
- [ ] Android: Internal testing track + License tester 로 결제 검증
- [ ] 환불 → coin 차감 webhook (RevenueCat → Cloudflare Worker) 필요 시 별도 endpoint 설계
