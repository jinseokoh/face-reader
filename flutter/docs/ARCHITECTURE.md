...
공개 설정(공개/비밀 PIN + 썸네일 공개) 스텝 플로우) · `TeamLobbyScreen`(슬롯 그리드 —
match 방은 남녀 2열, 썸네일 비공개면 성별 기본 아이콘 — + Realtime 반영 + QR + 초대
3버튼) · `BattleJoinScreen`(로그인→내 관상→PIN→사진 공개 계약→`join_battle`) ·
...
그리고, 최초 도달 클라이언트가 `submit_battle_result` 로 `result_payload` 를 1회
기록한다(first-writer-wins — 입력이 snapshot 으로 동결돼 있어 후착은 무해). 같은
트랜잭션이 베스트 쌍의 slot→user 를 resolve 해 `battle_matches` 에 upsert — 베스트
쌍 각자가 `respond_match` 로 수락/거절(1회, 재응답 불가)하고 둘 다 수락하면
`opened_at` 이 찍히며 `battle_messages` 인앱 1:1 채팅이 열린다. 이후 열람은
...
| `battle_matches` | team_id PK(cascade) · user_a/user_b · a_consent/b_consent · opened_at | 베스트 쌍 채팅 개설 상호 동의 — 배틀당 1행, opened_at = 둘 다 수락 시각 |
| `battle_messages` | id · team_id(→battle_matches cascade) · sender_id · body(≤500) · created_at | 성사된 쌍 전용 인앱 1:1 채팅. 수명 = teams 30일 purge cascade |
...
(match 방 성별 정원 = `GENDER_FULL`) · `leave_battle` · `submit_battle_result`(베스트
쌍 `battle_matches` upsert 포함) · `respond_match` · touch 트리거. View:
`public_battles`(모집 중 공개방 목록, 컬럼 화이트리스트) · `battle_roster`(team_members
...
`leave_battle`) — 직접 insert/update/delete 정책 없음. `battle_matches` 는 해당 쌍
본인만 select(타 참가자에게 동의 현황 비노출), 쓰기는 RPC 전용. `battle_messages` 는
