...
`ChemistryScreen`(2탭 — 공개 배틀/내 배틀) · `showBattleCreatePage`(방 이름→인원(4~12)→
공개/비밀(PIN)→연령대(전연령 또는 10~70대 RangeSlider)→공약 스텝 플로우) ·
`TeamLobbyScreen`(슬롯 그리드 + Realtime 반영 + QR + 초대 3버튼) · `BattleJoinScreen`
(로그인→내 관상→PIN→공약 동의→`join_battle`) · `TeamRevealScreen`(3-2-1 카운트다운 →
🏆 베스트 카드 → 공약 회수 카드 → N×N 밴드 매트릭스, 보는 사람 행 최상단 고정). 정원
충족이 유일한 시작 조건 — 방장 수동 시작 없음.
...
- `publicBattlesProvider`/`myBattlesProvider` — 서버 우선(로컬 캐시 없음). `BattleService`
  가 매 호출 `teams`/`public_battles`/`battle_roster` 를 직접 fetch, 갱신은
  `ref.invalidate`. 로그인만 하면 웹에서 조인한 배틀도 `team_members` 조회로 그대로
  뜬다 — 별도 rehydrate 불필요. 로비 화면은 이와 별개로 Supabase Realtime
...
이름 선등록이 아니라 `join_battle` RPC 셀프 조인이고, 정원 충족 시 같은 트랜잭션이
참가자 전원의 현재 my-face body 를 `teams.chemistry_snapshot` 에 동결하며 상태를
`revealing` 으로 전이한다(시작 후 재촬영이 결과에 영향을 못 주는 치팅 방어). 각
클라이언트는 그 snapshot 으로 `computeBattle`(shared/`compat/battle.dart`) 을 로컬
계산해 3-2-1 카운트다운 → 🏆 베스트 카드를 그리고, 최초 도달 클라이언트가
`submit_battle_result` 로 `result_payload` 를 1회 기록한다(first-writer-wins — 입력이
snapshot 으로 동결돼 있어 후착은 무해). 이후 열람은 `result_payload` 를 그대로 렌더.
...
| `teams` | id · owner_id · title · visibility · password · max_players · age_min/age_max · pledge · chat_url · status · started_at · closed_at · chemistry_snapshot · result_payload | 케미 배틀 방. status `recruiting→revealing→completed/expired`, closed_at+30일 = 수명 |
| `team_members` | id · team_id(cascade) · user_id(cascade) · slot_no · is_owner · joined_at | unique (team_id,user_id)·(team_id,slot_no). 쓰기는 RPC 전용(직접 insert/update/delete 정책 없음) |
...
`unlock_compat` · `ad_reward_status/record_view` · `handle_new_user` · `join_battle` ·
`leave_battle` · `submit_battle_result` · touch 트리거. View: `public_battles`(모집 중
공개방 목록, 컬럼 화이트리스트) · `battle_roster`(team_members + users.nickname, owner
권한 실행으로 닉네임만 노출).
...
owner, 상태 전이는 RPC 전용. `team_members` public read, 쓰기는 전부 RPC(`join_battle`/
`leave_battle`) — 직접 insert/update/delete 정책 없음. `coins`·`unlocks`·`ad_rewards`
self-read + RPC write. cron·탈퇴는 service-role.
