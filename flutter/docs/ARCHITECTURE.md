...
insert + 원장 기록). 결제 시 두 body 동결(self-contained) — metrics 소멸과 무관하게
`reconstructUnlockedPartners()` 복원. pair_key = 상대 supabaseId.
...
| `unlocks` | (user_id, pair_key) PK · owner_body · partner_body · total_score | self-contained 스냅샷 (의도된 보존) |
