...
상관 Monte Carlo 20,000 샘플(engine v2, 2026-04-18) 측정 결과, 속성별 raw p50:

| 속성 | p50 (male) | p90 | max |
|---|---|---|---|
| stability | 1.325 | 2.114 | 6.554 |
| trustworthiness | 0.969 | 1.394 | 3.781 |
| attractiveness | **0.683** | 1.223 | 4.684 (engine v1 대비 +0.39) |
| wealth | 0.141 | 0.803 | 6.047 |
| intelligence | 0.149 | 0.569 | 6.469 |
| sociability | 0.137 | 0.500 | 3.580 |
| emotionality | 0.200 | 0.517 | 2.219 |
| leadership | 0.070 | 1.111 | 10.876 |
| sensuality | 0.079 | 0.563 | 2.622 |
| libido | 0.026 | 0.495 | 3.521 |

원인:
- 9-node weight matrix 행 합은 =1.00 이지만 각 노드 입력 z-score 의 correlation 이 다름
- Stage 규칙 firing rate 가 속성마다 다름 (stability: Z-01 ≈11% 발동)
- Stage 0 shape preset 이 속성마다 다른 방향으로 기여 (oval→attractiveness +, square→leadership + 등)
- 결과: **동일 raw 0.5 가 stability 에선 저변, attractiveness 에선 p40 수준**. 속성 간 비교를 위해 quantile 필수.

**engine v2 에서 매력도 쏠림 해소**: 속성간 p50 최대편차가 engine v1 의 4.9× → 2.0× 수준으로 감소.
...
rank by globalPct (desc)                ← 얼굴 내 10 속성 랭킹
   ↓  rankPct = (9 - rank) / 9
blend = 0.40 × rankPct + 0.60 × globalPct    ← 2026-04-18 v10
   ↓
...
### 2.2 블렌드 비율 0.40 / 0.60 의 근거 (engine v2, 2026-04-18)

| 비율 | 효과 |
|---|---|
| 100% global | 모든 얼굴이 평균 근처 몰림 (표준편차 좁음) — 개인 차별화 실종 |
| 100% rank | 객관 위치 정보 소실 — "내가 전체 모집단 대비 어느 정도인지" 모름 |
| v9 (0.60 / 0.40) | 차별화 ↑ 였으나 **인위적 spread 강요** — attractiveness·trustworthiness 서열 강제 |
| **v10 (0.40 / 0.60)** | 상관 MC 로 calibration 정확도 올라간 이후, global fidelity 비중 ↑ 로 실 raw 반영 |

v10 은 최소 rank-기여 spread 가 2.0 (v9 의 3.0 → 2.0). calibration 이 정확해진 상태에서 인위적 서열 강요가 불필요해진 결과. `score_distribution_test` 의 invariant 도 3.0 → 2.0 으로 완화.
...
상관 Monte Carlo 20,000 샘플 (v10, engine v2) 결과 목표치:

| 지표 | 기대값 | 이유 |
|---|---|---|
| 출력 범위 | 5.0 ~ 10.0 | UI 표시용 5점 스케일 |
| 평균 top 속성 | ≥ 7.0 | rank=0 일 때 blend ≈ 0.4 + 0.6×gPct ≥ 0.4 → ≥ 7.0 |
| 평균 bottom 속성 | ≤ 7.0 | rank=9 일 때 blend = 0 + 0.6×gPct → ≤ 8.0 |
| top-bottom spread | ≥ 2.0 | 블렌드 0.4/0.6 구조적 보장 (rank-minimum) |
...
- base: N(0, 0.85)
- bone-latent N(0,1) × per-metric 로딩 (cheekbone/gonial/forehead 등 0.45~0.60)
- mid-latent N(0,1) × per-metric 로딩 (midFaceRatio 0.70 등)
- `koreanShapeDistribution` prior 주입 (oval 35% / oblong 18% / round 15% / square 12% / heart 10% / unknown 10%)
- Stage 0 shape preset 반영 (production 과 동일한 shape-aware MC)
...
- [OVERVIEW.md](../architecture/OVERVIEW.md) §2 — 상위 트리 엔진 맥락

---

## 연관 문서

- [OVERVIEW.md](../architecture/OVERVIEW.md) — 상위 아키텍처 (§2 Track 2, §4 Runtime Pipeline)
- [ATTRIBUTES.md](ATTRIBUTES.md) — weight matrix + rule 명세
- [TAXONOMY.md](TAXONOMY.md) — 14-node tree SSOT + 노드별 metric 매핑
