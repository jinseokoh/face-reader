...
final List<_Frag> _concludeOpening = [
  _Frag.hard(_yangStrong, [
    "당신의 얼굴은 양의 기운가 짙게 서린 상입니다. '@__PRIMARY_ARCHETYPE__' 의 골격 위에 '@__SECONDARY_ARCHETYPE__' 의 결이 얹혀 있지만, 그 모든 것을 관통하는 축은 강건·진취·돌파의 양기이며, 인생의 결정적 국면에서 머뭇거리지 않고 선을 넘어서는 기질이 당신의 궤적을 만듭니다.",
  ]),
  _Frag.hard(_yinStrong, [
    "당신의 얼굴은 음의 기운가 @{deep} 깃든 상입니다. '@__PRIMARY_ARCHETYPE__' 위에 '@__SECONDARY_ARCHETYPE__' 의 결이 흐르지만, 전체를 감싸는 기운은 수렴·포용·유연의 음기이며, 서두르지 않고 시간을 동맹으로 삼는 결이 당신의 평생 자산입니다.",
  ]),
  _Frag.hard(_yyHarmony, [
    "당신의 얼굴은 음과 양이 @{rare} 고르게 맞물린 조화의 상입니다. '@__PRIMARY_ARCHETYPE__' 과 '@__SECONDARY_ARCHETYPE__' 이 겹친 위에, 강함과 부드러움을 자유롭게 교체할 수 있는 중용의 결이 자리하여, 어떤 환경에서도 자기 자리를 빨리 찾는 적응력이 최대 강점이 됩니다.",
  ]),
...
final List<_Frag> _concludeStage = [
  ...
  _Frag.hard((f) => f.age.isOver30 && !f.age.isOver50, [
    '당신의 현재 단계에서 관상이 강조하는 지점은 \'축적의 설계\'입니다. 초기의 재능이 드러난 시기이고, 지금부터 10년은 그 재능을 어떤 시스템 위에 올려놓느냐가 평생 곡선의 기울기를 결정합니다. \'중년에 운이 크게 열리는 시기\'의 기반이 만들어지는 구간이기에, 작은 선택들이 복리처럼 쌓여 5~7년 뒤 전혀 다른 풍경을 만들어냅니다.',
  ]),
...
final List<_Frag> _healthAdvice = [
  ...
  _Frag.hard((f) => true, [
    ...
    '수명의 축을 관상학은 "정신·기운·체력" 셋으로 나눕니다. 당신의 얼굴에서 가장 먼저 흐려지는 축을 알아채는 사람은 이 셋을 따로따로 충전하는 방법을 가지고, 한 축으로만 충전하려는 사람은 결국 다른 둘이 말라갑니다. 잠은 신을, 호흡은 기를, 식사는 정을 보충합니다.',
...
final List<_Frag> _healthOpening = [
  ...
  _Frag.hard((f) => true, [
    ...
    '얼굴 전체가 뚜렷한 편중 없이 얼굴 세 부분에 고루 기운을 나누어 싣고 있는 결입니다. 큰 병보다 잔잔한 누적이 두드러지는 유형—매일의 작은 루틴 하나가 20년 뒤의 체감 연령을 그대로 결정합니다.',
    ...
    '몸의 기운이 한쪽으로 쏠리지 않고 여러 축으로 분산된 결은 관상학에서 "균형 잡힌 몸"라 부릅니다. 한 가지 결정적 장점이 없는 대신 치명적 약점도 늦게 드러나는 유형—10년, 20년 단위의 비교에서 진가가 나오는 구조입니다.',
...
final List<_Frag> _healthStrength = [
  ...
  _Frag.hard((f) => f.fired('Z-09'), [
    '이마 부근의 기운이 @{intense} 강한 상은 머리를 많이 쓰는 기질을 의미합니다. 수면의 질이 건강의 어떤 요소보다 먼저 흔들리기 쉬운 유형입니다.',
  ]),
  ...
  _Frag.hard((f) => f.fired('Z-04'), [
    '턱 부근이 후중한 상은 위장·신장 쪽의 근기가 좋은 결로 읽힙니다. 식습관의 축적이 가장 정직하게 수명으로 환원되는 타입입니다.',
  ]),
  ...
  _Frag.hard((f) => true, [
    ...
    '관상학에서 "기운과 혈색이 고르게 흐르는 인상"이라 부르는 결입니다. 큰 파고가 없는 대신 섬세한 유지가 필요한 유형이며, @__STRONGEST_NODE__의 결이 건강 전반의 리드미컬한 지표가 됩니다.',
...
final List<_Frag> _romanceAdviceMale = [
  _Frag(_highPair(Attribute.attractiveness, Attribute.emotionality), [
    '매력과 감정의 해상도가 동시에 짙은 결은 관상이 "매혹형"이라 부르는 @{rare} 구조입니다. 다만 이 결은 자기 관리가 없으면 에너지가 사방으로 분산되기 쉬우니, 하나의 관계를 깊이 파는 훈련이 평생 연애의 상한을 결정합니다.',
  ]),
  _Frag(_bandPair(Attribute.attractiveness, _Band.high, Attribute.emotionality, _Band.mid), [
...
final List<_Frag> _romanceOpeningFemale = [
  ...
  _Frag(_bandPair(Attribute.attractiveness, _Band.high, Attribute.emotionality, _Band.high), [
    '당신의 연애는 \'불러오고, 깊게 읽는\' 이중 역학입니다. @{palace_mate}이 열려 여러 방향에서 호감이 들어오고 눈 밑의 윤기가 상대의 속결까지 @{intense} 간파합니다. 선택지의 폭과 해석의 밀도가 동시에 높아 \'이 사람이 맞나\' 를 증명하려는 단계가 깊어지는 결입니다.',
  ]),
  _Frag(_bandPair(Attribute.attractiveness, _Band.high, Attribute.emotionality, _Band.mid), [
  ...
  _Frag(_bandPair(Attribute.attractiveness, _Band.low, Attribute.emotionality, _Band.high), [
    '당신의 연애는 \'늦게 빛나는\' 깊이의 결입니다. 외형의 파장이 즉시 끌어당기는 결은 아니나 한 번 대화를 나눈 상대가 며칠 뒤 당신을 다시 떠올리는 여운형이며, 감수성의 밀도가 평균을 크게 뛰어넘는 @{noble_f} 연애를 만듭니다.',
  ]),
  _Frag(_bandPair(Attribute.attractiveness, _Band.low, Attribute.emotionality, _Band.mid), [
  ...
  _Frag(_bandPair(Attribute.attractiveness, _Band.low, Attribute.emotionality, _Band.low), [
    '당신의 연애는 \'동지 결합\' 의 결에 가깝습니다. 뜨거운 구애보다 같은 가치·같은 방향을 확인한 상대와 조용히 나란히 걷는 형태이며, 결혼이라는 결론에 도달하는 직진성이 평균보다 @{intense} 강합니다.',
  ]),
  _Frag.hard((f) => true, [
...
final List<_Frag> _romanceOpeningMale = [
  ...
  _Frag(_bandPair(Attribute.attractiveness, _Band.low, Attribute.emotionality, _Band.low), [
    '당신의 연애는 \'자기 세계 주도\' 형에 가깝습니다. 화려한 구애나 감정 곡예 없이 자신의 일·목표·루틴을 단단히 가진 상대에게 조용히 끌리는 결이며, 결혼 상대를 고르는 기준이 일찍 확정되는 직선성이 특징입니다.',
  ]),
  _Frag.hard((f) => true, [
...
final List<_Frag> _romanceStrengthFemale = [
  _Frag.hard((f) => f.fired('P-08'), [
    '@{palace_sex} 아래 눈 밑의 윤기가 살아 있는 구조는 매력이 강해지는 시기가 규칙적으로 돌아오는 결로, 한 해에 한두 번 의미 있는 인연의 문이 열리는 주기성이 자리합니다.',
  ]),
  _Frag.hard((f) => f.fired('L-EL'), [
    '측면에서 입술선이 도톰하게 드러나는 상은 관상학의 \'매력의 기색\' 이며, 상대의 시선이 당신의 입매에 오래 머무는 @{subtle} 매력을 만듭니다.',
  ]),
  _Frag.hard((f) => f.bandOf(Attribute.trustworthiness) == _Band.high, [
    '언행이 일치하는 상은 \'한 번 정하면 끝까지 가는\' 신뢰를 상대에게 각인시키며, 장기 관계의 뿌리를 @{deep} 박게 하는 @{noble_f} 덕이 서려 있습니다.',
  ]),
  _Frag.hard((f) => f.bandOf(Attribute.emotionality) == _Band.high, [
...
final List<_Frag> _romanceStrengthMale = [
  ...
  _Frag.hard((f) => f.fired('O-CK') || f.nodeZ('cheekbone') >= 0.8, [
    '@{mount_e}·@{mount_w}이 받쳐주는 구조는 \'기백\' 이 실린 결로, 당신이 들어서는 순간 공기의 중심이 옮겨 가는 결이 연애의 출발점에서 @{intense} 작동합니다.',
  ]),
  ...
  _Frag.hard((f) => f.nodeZ('chin') >= 0.5, [
    '@{mount_n}이 단정한 상은 \'한 번 정하면 끝까지 간다\' 는 턱 끝의 결이며, 흔들리지 않는 뿌리가 장기 관계의 가장 큰 축이 됩니다.',
  ]),
  _Frag.hard((f) => true, [
...
final List<_Frag> _socialAdvice = [
  ...
  _Frag.hard((f) => true, [
    ...
    '당신에게 가장 중요한 사람 다섯 명을 종이에 쓰고 각각에게 이번 달 얼마의 시간을 썼는지 세어 보십시오. 그 숫자가 관계의 진짜 지도입니다. @__STRONGEST_NODE__의 기운이 아무리 좋아도 이 지도를 안 그리는 사람이 받는 노년 복은 얇아지는 @{structure}입니다.',
...
final List<_Frag> _socialOpening = [
  ...
  _Frag(_lowPair(Attribute.sociability, Attribute.trustworthiness), [
    '사람을 여는 문도, 오래 품는 문도 @{faint} 좁은 결입니다. 관상학에서 "고립형 인상"이 살짝 드러나는 유형—혼자 쌓는 시간에서 진짜 결과물이 나오는 독립형 @{structure}입니다.',
  ]),
  _Frag.hard((f) => true, [
    ...
    '관상학에서 "마음이 오래 머무는 얼굴"이라 부르는 결. 첫 만남의 임팩트보다 여러 번 만난 뒤 드러나는 "편안함"이 당신 관계의 진짜 힘—중년 이후 남은 사람들이 말하는 "신뢰의 중심"에 당신이 서 있게 되는 @{structure}입니다.',
  ]),
];
...
final List<_Frag> _socialStrength = [
  ...
  _Frag.hard((f) => f.fired('L-SN'), [
    '들창코의 결은 관상학에서 "사교의 기운"가 열려 있다고 보는 신호로, 낯선 자리에 섞여드는 속도가 남다르게 빠른 기질입니다.',
  ]),
  ...
  _Frag.hard((f) => f.nodeZ('eyebrow') >= 0.5, [
    '눈썹이 정돈된 상은 관상학에서 "형제 운"이 살아 있는 결로, 또래·동료 관계에서 중재자 역할이 자연스럽게 주어지는 기질입니다.',
  ]),
  _Frag.hard((f) => true, [
    '당신의 관계는 "오래가는 소수"와 "스치듯 지나가는 다수" 사이의 분리가 또렷한 구조로, 시간이 지날수록 핵심 그룹의 밀도가 @{deep} 짙어지는 결을 가졌습니다.',
    '관상학에서 "사람과 잘 어울리는 기운"이라 부르는 결—말을 많이 하지 않아도 주변 사람들이 자연스럽게 당신을 중재자 자리에 앉히는 구조입니다. @__STRONGEST_NODE__의 결이 이 인화의 에너지를 받쳐주는 중심축입니다.',
    '당신은 "감정의 완급"을 조절하는 능력이 남보다 반걸음 앞선 결입니다. 상대의 기분을 먼저 읽고 자기 반응의 온도를 맞추는 감각이 자연스럽게 갖춰진 상—이는 사회적 마찰을 미리 줄이는 @{rare} 기질의 신호입니다.',
    '@{palace_home}의 기운이 단단하게 자리잡은 상은 "환대의 인상"로 읽힙니다. 당신의 공간·시간·정성을 다른 사람에게 열어주는 방식이 남보다 덜 인색한 구조—이것이 장기적으로 관계의 저수지를 깊게 만드는 힘입니다.',
...
final List<_Frag> _talentOpening = [
  _Frag(_highPair(Attribute.intelligence, Attribute.leadership), [
    '머리와 발이 동시에 굵게 박힌 얼굴입니다. 관상학이 "지혜로운 장수형"이라 부르는 드문 결—판을 먼저 읽고 그 판 위에 직접 올라가 흐름을 돌리는 @{rare} 조합. @{palace_destiny}의 열림과 @{palace_career}의 묵직함이 한 얼굴 안에서 동시에 호흡합니다.',
  ...
  _Frag.hard((f) => true, [
    '@{talent_word}이 한 방향으로 쏠리지 않고 여러 영역에 고루 잠재된 형태입니다. 얼굴 세 부분이 균형을 이룬 상으로, @__STRONGEST_NODE__의 결이 가장 @{intense} 드러나며 이 부위가 관여하는 영역에서 또래보다 반 걸음 앞서는 감각이 붙어 있습니다.',
    ...
    '특정 부위가 혼자 튀지 않고 여러 기관이 함께 합주하는 결입니다. 관상학이 "균형 잡힌 기운"라 부르는 상으로, 한 영역의 극단적 성취보다 여러 영역을 묶어내는 감각이 @{talent_word}의 본체로 작동하는 @{structure}입니다.',
...
final List<_Frag> _talentShadow = [
  ...
  _Frag(_lowOf(Attribute.stability), [
    '다만 흥미가 이동하는 속도가 빠른 편입니다. 관상학이 "이마 부근이 너무 강한 인상"이라 부르는 기질—머리가 먼저 달려가 몸이 따라잡지 못하면 재능이 쌓이지 않고 흩어지는 구조입니다.',
  ]),
...
final List<_Frag> _talentStrength = [
  ...
  _Frag.hard((f) => f.fired('P-02') || f.nodeZ('forehead') >= 1.0, [
    '@{open_wide} 반듯한 이마는 관상학이 "이마 위쪽이 시원하게 열린 인상"이라 부르는 상으로, 윗사람의 도움과 윗선의 기회가 먼저 찾아오는 기질을 @{intense} 암시합니다.',
  ...
  _Frag.hard((f) => f.fired('A-02'), [
    '이마 기운의 열림은 "이른 나이에 빛나는 인상"의 신호로, 젊은 시기의 도약이 동년배보다 앞서 찾아오는 기질을 의미합니다.',
  ]),
  _Frag.hard((f) => true, [
    '당신의 기질은 화려한 한순간의 폭발보다 시간에 정직하게 비례하는 축적형—관상학이 "늦게 크는 사람"이라 부르는 결에 가깝습니다.',
    ...
    '당신의 얼굴에는 "결과보다 과정을 끝까지 책임지는" 기질이 새겨져 있습니다. 관상학에서 "후덕한 인상"이라 부르는 상—한 프로젝트의 마지막 20% 잔무를 끝까지 물고 가는 뚝심이 동년배가 가장 부러워하는 자산이 됩니다.',
    '@__STRONGEST_NODE__과 @__SECOND_NODE__가 나란히 자리한 상은 "두 개의 엔진"이 동시에 도는 구조입니다. 한쪽이 지칠 때 다른 쪽이 받치는 두 갈래로 작동하는 구조—단일 엔진의 사람들보다 피크가 늦게 오지만 정체 구간이 훨씬 짧은 @{rare} 기질입니다.',
  ]),
];

final List<_Frag> _wealthAdvice = [
  _Frag(_highPair(Attribute.wealth, Attribute.stability), [
    '버는 감각과 지키는 뚝심이 한 몸에 박힌 "큰 재물형"의 결입니다. 관상이 약속한 상한은 분명—이 재능을 복리로 돌리려면 자산의 구조에 집중하십시오. 월급보다 매달 자동으로 쌓이는 액수가 진짜 재물의 크기이며, 남의 돈·남의 시간을 다루는 경험이 30대에 들어오면 평생 곡선이 열립니다.',
  ]),
...
final List<_Frag> _wealthOpening = [
  _Frag(_highPair(Attribute.wealth, Attribute.stability), [
    '돈이 머물고 싶어하는 구조가 골상에 새겨진 "큰 재물"의 결입니다. @{palace_wealth}이 @{strong_adj} 자리하고 @{mount_n}이 함께 받쳐주는 상—버는 감각과 지키는 뚝심이 한 얼굴에 공존하는 @{rare} 조합으로, 사업·투자·운영 어느 경로로 가도 "남기는 사람"이라는 평이 따라붙습니다.',
  ]),
  _Frag(_bandPair(Attribute.wealth, _Band.high, Attribute.stability, _Band.mid), [
  ...
  _Frag.hard((f) => true, [
    ...
    '@{palace_wealth}이 쏠림 없이 차분하게 자리한 결입니다. 화려하게 한 번에 풀리는 운보다 꾸준한 축적에서 진가가 드러나는 유형으로, 누적 시간이 평생 재산의 규모를 그대로 반영하는 @{structure}입니다.',
    '관상학이 "정직한 재물형"이라 부르는 상입니다. 큰 한 방보다 성실한 반복으로 재물을 쌓는 기질—근로·장기 저축·부동산 같은 정직한 트랙에서 평균 이상의 결실이 붙는 @{rare} 구조가 얼굴에 새겨져 있습니다.',
    '@{mount_c}이 균형 있게 자리잡은 상. 재물에 대한 집착이 과하지도 얕지도 않은 중용의 결이라, 감정이 판단에 끼어드는 지점이 상대적으로 적고 장기 복리가 작동하기 좋은 구조입니다.',
    '당신의 재물 결은 "큰 행운"에 기대지 않고 "작은 정직"에 누적되는 @{structure}입니다. 관상학이 40~50대에 말하는 "중년에 운이 크게 열리는 흐름"이 가장 자연스럽게 들어오는 유형—조급함 없이 시간을 자산으로 환원하는 훈련이 평생 노년의 풍요를 결정합니다.',
  ]),
];

final List<_Frag> _wealthShadow = [
  _Frag(_highPair(Attribute.wealth, Attribute.stability), [
    '다만 "잘 버는데 잘 안 쓰는" 결이 굳어지면 관계 안에서 인색함으로 비쳐 자산의 크기에 비해 노년에 누리는 풍요는 얇아지기 쉽습니다. 돈과 마음을 함께 쓰는 설계가 있어야 재물운이 다음 세대까지 이어집니다.',
  ]),
  ...
  _Frag.hard((f) => true, [
    ...
    '다만 당신의 재물운은 "들어올 때의 크기"보다 "빠져나갈 때의 크기"가 더 넓게 열린 결입니다. 관상학이 "기운이 새는 인상"라 부르는 상—작은 새는 구멍 여럿이 모여 한 해 총 순유입의 상당 부분을 갉아먹는 구조이니, 고정비·구독·소액 지출을 주기적으로 점검하는 습관이 결정적 방어선입니다.',
...
final List<_Frag> _wealthStrength = [
  _Frag.hard((f) => f.fired('P-06') || f.nodeZ('nose') >= 1.0, [
    '@{mount_c}이 @{strong_adj} 자리잡은 구조는 관상학이 "중년에 운이 크게 열리는 시기"이라 부르는 신호—30대 후반에서 40대 사이에 재물의 꼭짓점이 찾아오는 기질을 @{intense} 암시합니다.',
  ]),
  _Frag.hard((f) => f.fired('O-CK') || f.nodeZ('cheekbone') >= 0.8, [
  ...
  _Frag.hard((f) => f.fired('Z-04') || f.nodeZ('chin') >= 1.0, [
    '@{mount_n}이 @{strong_adj} 두터운 구조는 "말년재운"의 상징입니다. 50대 이후에도 재물의 뿌리가 마르지 않고 오히려 @{deep} 깊어지는 노년 풍요의 결입니다.',
  ]),
  _Frag.hard((f) => f.fired('Z-11'), [
    '얼굴 중간 부분의 기운이 @{intense} 열린 상은 중년에 운이 한꺼번에 풀리는 신호로, 가장 큰 결실이 인생의 중간에 집결되는 리듬을 뜻합니다.',
  ]),
  ...
  _Frag.hard((f) => true, [
    ...
    '관상학에서 "재물의 기운이 얼굴 전체에 고르게 깔린 인상"이라 부르는 결입니다. 한 분야에 몰빵하지 않고 여러 소득원이 함께 돌아가는 구조가 자연스럽게 맞는 기질—"N잡"의 시대와 결이 일치하는 @{rare} 유형입니다.',
