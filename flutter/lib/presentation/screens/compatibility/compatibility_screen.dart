        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              const Text(
                '옛 관상학이 두 사람의 결을 볼 때는, 인생의 어느 자리에서 어떻게 부딪히는지를 따로따로 따져 무게를 둡니다. 네 차원의 비중도 그 가르침을 따른 것입니다.\n\n'
                '• 12 가지 생활 영역 40% — 十二宮(십이궁): 부부(夫妻)·재물(財帛)·자녀(子女)·관록(官祿)·질액(疾厄)·천이(遷移)·노복(奴僕)·전택(田宅)·복덕(福德)·부모(父母)·형제(兄弟)·명궁(命宮). 결혼·돈·자녀·일·건강·이동·인덕·부동산·복·부모·형제·운명 — 두 사람이 평생 부딪히는 실생활의 결이 이 12 자리에 모두 들어와 있어 가장 무거운 무게를 둡니다. 五行(가치관)이 큰 토대라면, 十二宮은 그 토대 위에서 매일·매년 마주하는 결.\n'
                '• 소통 스타일 25% — 五官(눈·코·입·귀·눈썹)이 만들어내는 표현 방식. 매시간 부딪히는 신호라 단기 호흡·갈등의 가장 빠른 1차 지표.\n'
                '• 가치관 20% — 五行 기반 평생 변하지 않는 큰 결. 토대로서의 무게는 크되 십이궁만큼 세분화되지 않습니다.\n'
                '• 이성적 끌림 15% — 결의 깊이가 아니라 색(色)의 강도를 보는 자리. 동성·소년·노년 페어에서는 의미가 옅어지므로 평균 가중치를 가장 낮게.\n\n'
                '균등 25% 가 공평해 보일 수 있으나, 그것은 네 차원이 같은 무게로 사람을 흔든다는 가정 위에 있습니다. 옛 술서와 데이터 모두 그렇지 않다는 것을 같은 결로 일러 옵니다.',
                style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.w400,
                  color: AppTheme.textSecondary,
