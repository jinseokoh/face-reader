...
final compatibilityKeysProvider = FutureProvider.autoDispose<Set<String>>((ref) {
...
/// compatibilities 스냅샷에서 복원한 **내 쌍** 파트너 리포트 목록 (로컬에 없는 갭
/// 메우기용 — 매칭 제3자 쌍은 제외). auth 변화에 재구독.
final compatibilityPartnerBodiesProvider =
    FutureProvider.autoDispose<List<FaceReadingReport>>((ref) async {
      ref.watch(authProvider);
      return CompatibilityService().reconstructPartners(
...
final compatibilityPairsProvider = FutureProvider.autoDispose<List<CompatibilityPair>>((
  ...
  ref.watch(authProvider);
  return CompatibilityService().pairs();
});
