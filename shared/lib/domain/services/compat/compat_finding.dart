...
///
/// 설계 원칙 (사용자 지침 준수):
///   - 모든 text 는 한국어. 한자는 ( ) 안 풀이에만.
///   - "기운이 흐른다" 같은 추상 금지. 실제 행동·갈등 상황으로 기술.
///   - 분석가 톤, 감성팔이 금지.
library;
...
const Map<ElementRelationKind, _FindingTemplate> _elementTemplate = {
  ElementRelationKind.generating: _FindingTemplate(
    ...
    caution: '베푸는 쪽이 지치지 않도록, 상대의 감사·반응이 중요합니다.',
  ...
  ElementRelationKind.generated: _FindingTemplate(
    ...
    action: '받는 쪽이 주기적으로 먼저 챙기는 이벤트(식사·여행·선물)를 기획해 균형을 맞춰 주시기 바랍니다.',
  ),
  ElementRelationKind.overcoming: _FindingTemplate(
    ...
    action: '결정을 밀어붙이기 전에 "너는 어떻게 생각해?" 를 먼저 묻고, 상대의 답을 반드시 듣는 규칙을 만드시기 바랍니다.',
  ),
  ElementRelationKind.overcome: _FindingTemplate(
    ...
    action: '내 의견을 말할 수 있는 "안전한 대화 시간" 을 주 1회 고정하고, 그 시간엔 상대가 반박 없이 들어 주기로 약속하시기 바랍니다.',
  ),
  ElementRelationKind.identity: _FindingTemplate(
    ...
    action: '일부러 서로 다른 영역의 취미나 친구 모임을 갖고, 그 경험을 서로에게 공유하는 습관을 들이시기 바랍니다.',
  ),
};
...
const Map<String, _FindingTemplate> _organTemplate = {
  'OP-BR-BOTH-THICK': _FindingTemplate(
    ...
    action: '영역별로 "결정권 가진 사람" 을 미리 정해 두시기 바랍니다 (예: 인테리어는 A, 재정은 B).',
  ),
  'OP-BR-THICK-THIN': _FindingTemplate(
    ...
    action: '주요 결정 후에는 "너는 어떻게 생각해?" 한마디를 의식적으로 먼저 물으시기 바랍니다.',
  ),
  'OP-EY-FENG-TAOHUA': _FindingTemplate(
    ...
    action: '가끔씩 역할을 바꿔 보시기 바랍니다 — 감성 쪽이 결정하고, 이성 쪽이 감정 표현을 주도하는 식으로.',
  ),
  'OP-EY-DROOPING-BOTH': _FindingTemplate(
    ...
    action: '둘 중 한 명이 "이번 달 담당" 으로 돌아가며 외부 활동을 기획하는 규칙을 만들어 두시기 바랍니다.',
  ),
  'OP-EY-SHARP-SOFT': _FindingTemplate(
    ...
    action: '서로의 역할을 약점이 아니라 강점으로 명확히 언어화해 두시기 바랍니다.',
  ),
  'OP-NS-BOTH-AQUILINE': _FindingTemplate(
    ...
    action: '큰 금액 결정은 제3자(회계사·가족 어른)에게 기준 의견을 받아와 객관적 숫자로 합의하시기 바랍니다.',
  ),
  'OP-NS-AQUI-SNUB': _FindingTemplate(
    ...
    action: '바쁜 시기에도 "짧지만 자주" 원칙으로 연락과 스킨십을 유지하시기 바랍니다.',
  ),
  'OP-NS-GARLIC-BOTH': _FindingTemplate(
    ...
    action: '월 수입의 일정 비율(예: 10%)은 반드시 즐기는 데 쓰도록 "즐거움 예산" 을 고정해 두시기 바랍니다.',
  ),
  'OP-NS-THIN-BOTH': _FindingTemplate(
    ...
    action: '소액 지출은 모두 공동 카드 하나로 몰아 자동 정산하고, 1만원 이하는 따지지 않는 규칙을 합의하시기 바랍니다.',
  ),
  'OP-MO-BOTH-FULL': _FindingTemplate(
    title: '대화와 감각의 맞물림',
    domain: '성향·대화',
    meaning: '입술이 도톰하다는 건 감각·표현이 풍부하다는 뜻이고, 이게 겹치면 대화가 메마르지 않습니다.',
    strength: '평소 말·스킨십·애정 표현이 자연스러워 관계의 온도가 높게 유지됩니다.',
    caution: '말의 온도에 의존하다 보면 행동의 일관성을 놓치기 쉽습니다.',
    action: '말로 한 약속은 반드시 행동으로 확인하는 루틴을 만들어 두시기 바랍니다.',
  ),
  'OP-MO-BIG-SMALL': _FindingTemplate(
    ...
    action: '말이 많은 쪽이 의식적으로 "네 생각은?" 으로 질문의 턴을 넘기시기 바랍니다.',
  ),
  'OP-MO-WIDE-SMILE': _FindingTemplate(
    ...
    action: '"오늘 힘들었던 일" 을 일부러 꺼내는 시간을 주 1회 만들어 두시기 바랍니다.',
  ),
  'OP-MO-CORNER-DOWN': _FindingTemplate(
    ...
    action: '말로 꺼내기 어려우면 메모·문자로라도 매일 "오늘 기분 상태" 한 줄을 공유하는 규칙을 만드시기 바랍니다.',
  ),
  'OP-MO-CHERRY-SMALL': _FindingTemplate(
    title: '표현의 위축',
    domain: '성향·대화',
    meaning: '입이 작고 얇다는 건 감정·의견 표현에 소극적이라는 뜻이고, 이게 겹치면 서로의 속을 알기 어렵습니다.',
    ...
    action: '원하는 것·싫은 것을 말할 때는 "나는 ~를 원해/싫어" 식의 직설 표현을 쓰기로 규칙을 정하시기 바랍니다.',
  ),
};
...
const Map<String, _FindingTemplate> _palaceTemplate = {
  'PP-SP-STRONG-SMOOTH': _FindingTemplate(
    title: '부부로서의 결속력',
    domain: '결혼·부부생활',
    meaning: '눈꼬리 옆쪽(부부궁)은 결혼 생활에서 상대를 얼마나 존중하고 금슬을 오래 유지할 수 있는지를 보는 자리입니다.',
    strength: '서로를 귀하게 여기는 태도가 몸에 배어 있어, 결혼하면 안정적인 반려 관계를 오래 유지합니다.',
    caution: '궁합이 좋은 구간일수록 방심하기 쉬우니, 기념일·연락·스킨십 같은 기본 관리는 꾸준히 해 주시기 바랍니다.',
    action: '좋은 부부운을 당연시하지 말고, 매달 "이번 달에 고마웠던 일 세 가지" 정도는 말로 꺼내 두십시오.',
  ),
  'PP-SP-STRONG-NOFLAG': _FindingTemplate(
    title: '결혼 중심의 삶',
    domain: '결혼·부부생활',
    meaning: '부부궁이 두 분 모두 단단하다는 건, 결혼 자체가 인생에서 큰 비중을 차지하게 된다는 뜻입니다.',
    strength: '연애보다 결혼 쪽에 더 잘 맞는 두 사람이라, 정식 관계로 들어갔을 때 안정성이 높습니다.',
    caution: '결혼을 인생의 중심에 두는 성향이 겹쳐, 한쪽이 일·취미 쪽에 치우치면 실망이 큽니다.',
    action: '서로의 연간 계획을 공유하고, 결혼 생활 외 영역에 쏟는 시간을 미리 합의해 두시기 바랍니다.',
  ),
  'PP-SP-CROSS': _FindingTemplate(
    title: '부부로서의 결속력',
    domain: '결혼·부부생활',
    meaning: '부부궁의 강도가 한쪽이 강하고 한쪽이 약하면, 관계에 대한 기대치 차이가 큽니다.',
    strength: '강한 쪽이 약한 쪽을 품어 주면 오히려 균형이 잡혀 차분히 안착하는 모양입니다.',
    caution: '강한 쪽이 상대의 소극성을 불만으로 쌓아 두면, 어느 순간 결혼 유지 여부 자체를 문제 삼게 됩니다.',
    action: '관계 온도 차이를 숫자(예: 1~10점)로 주기적으로 공유하고, 크게 벌어지면 바로 대화로 풀어 주시기 바랍니다.',
  ),
  'PP-SP-WEAK-WRINKLE': _FindingTemplate(
    title: '부부로서의 결속력',
    domain: '결혼·부부생활',
    meaning: '두 분 모두 부부궁이 약한 데다 눈꼬리 주름이 겹친 건, 부부로서의 감정 기복이 크다는 신호입니다.',
    strength: '약점을 공유하기 때문에 서로 공감은 빠른 편입니다.',
    caution: '잔주름처럼 쌓인 서운함이 폭발하면 이혼·별거 같은 극단 선택으로 가기 쉽습니다.',
  ...
  'PP-SP-WEAK-NOFLAG': _FindingTemplate(
    title: '부부로서의 결속력',
    domain: '결혼·부부생활',
    meaning: '부부궁이 양쪽 다 약한 건, 결혼·장기 관계에서 에너지가 쉽게 식는다는 뜻입니다.',
    strength: '서로에게 큰 기대가 없어, 기대와 현실의 격차로 상처받는 일은 적은 편입니다.',
    caution: '감정 표현이 부족해 "함께 사는 룸메이트"처럼 식어 버릴 위험이 있습니다.',
    scenario: '일·육아에 치여 하루 대화가 10분도 안 되는 날이 이어지면서, 어느 순간 서로의 근황조차 모르는 상태가 됩니다.',
    action: '정해진 시간에 무조건 하루 20분 얼굴 보고 대화하는 루틴을 만들어 두시기 바랍니다.',
  ),
  'PP-CH-STRONG-PLUMP': _FindingTemplate(
    title: '자녀운과 가까운 관계의 온기',
    domain: '자녀·친밀',
    meaning: '눈 아래 볼록한 부분(남녀궁·애교살)은 자녀, 연인, 가까운 사람을 향한 애정을 보는 자리입니다.',
    strength: '자녀를 향한 정성과 가까운 이를 돌보는 태도가 자연스럽게 두 분 모두에서 나옵니다.',
    caution: '애정이 풍부한 만큼, 한쪽이 다른 쪽의 애정 대상(자녀·부모)에 질투심을 느낄 수 있습니다.',
    action: '둘만의 시간과 가족 돌보는 시간을 달력에 구분해 기록해 두고, 어느 한쪽이 지나치게 쏠리지 않도록 관리하시기 바랍니다.',
  ),
  'PP-CH-WEAK-HOLLOW': _FindingTemplate(
    ...
    action: '하루 한 번 "고마워", "사랑해" 같은 직설적 표현을 억지로라도 하는 습관을 들이셔야 합니다.',
  ),
  'PP-LF-BRIGHT-BOTH': _FindingTemplate(
    title: '삶의 방향과 결단력',
    domain: '삶의 방향·결단',
    meaning: '미간(명궁)이 밝고 넓다는 건, 인생의 큰 결정을 스스로 밀고 나갈 기개가 있다는 의미입니다.',
    strength: '이사·이직·투자 같은 큰 결정을 두고 서로의 판단을 신뢰하고 빠르게 합의합니다.',
    caution: '둘 다 추진력이 강해, 급한 결정으로 들어가 주변 조언을 듣지 않을 위험이 있습니다.',
    action: '큰 결정 전에 "24시간 멈춤 규칙"을 두고, 가까운 제3자 한 명의 의견을 반드시 확인하시기 바랍니다.',
  ),
  'PP-LF-TIGHT-BOTH': _FindingTemplate(
    ...
    action: '매주 정해진 요일에 20분 "이번 주에 마음에 걸린 일" 만 꺼내는 시간을 고정해 두시기 바랍니다.',
  ),
  'PP-LF-CROSS': _FindingTemplate(
    ...
    action: '큰 결정 앞에서는 "검토 기간 X주" 를 숫자로 먼저 합의한 뒤 그 안에서 움직이는 규칙을 두시기 바랍니다.',
  ),
  'PP-WE-BULB-BOTH': _FindingTemplate(
    ...
    action: '연간 저축 목표와 함께 "즐기기 위한 예산"도 반드시 숫자로 합의해 두시기 바랍니다.',
  ),
  'PP-WE-HOOK-CLASH': _FindingTemplate(
    ...
    action: '공동 계좌·개인 계좌 비율을 명확히 나누고, 일정 금액 이상의 지출은 사전 승인하는 규칙을 문서로 남기시기 바랍니다.',
  ),
  'PP-WE-THIN-BOTH': _FindingTemplate(
    ...
    action: '공동 지출 항목을 앱 한 개로 통합해 영수증 단위로 자동 정산하고, 1만원 이하 지출은 서로 따지지 않기로 규칙을 정하십시오.',
  ),
  'PP-TH-STRONG': _FindingTemplate(
    title: '생활 기반 만들기',
    domain: '생활 기반',
    meaning: '눈과 눈썹 사이(전택궁)가 두텁다는 건, 집·부동산·가정의 안정에 대한 감각이 살아 있다는 뜻입니다.',
    strength: '집 마련·정착 시점을 비슷하게 설정하기 쉬워, 주거 문제로 다툴 일이 적습니다.',
    caution: '정착 성향이 강해, 새로운 기회를 놓치고 과하게 보수적으로 운영될 가능성이 있습니다.',
    action: '5년·10년 주기로 이사·리모델링 같은 변화 계획을 의식적으로 세워 두시기 바랍니다.',
  ),
  'PP-TH-WEAK': _FindingTemplate(
    title: '생활 기반의 취약성',
    domain: '생활 기반',
    meaning: '전택궁이 얇다는 건, 주거·가정 기반이 불안정할 때 관계까지 흔들리기 쉽다는 신호입니다.',
    ...
    action: '비상자금(최소 3개월 생활비)과 주거 계획(1~3년짜리)을 문서로 만들어 두시기 바랍니다.',
  ),
  'PP-FT-CLOUDLESS': _FindingTemplate(
    title: '일상의 여유',
    domain: '복·여유',
    meaning: '이마 위 좌우(복덕궁)가 밝다는 건, 일상에서 여유를 느끼는 감각이 두 분 다 살아 있다는 뜻입니다.',
    strength: '여행·취미·친구 만남 같은 여유 영역이 관계를 건강하게 받쳐 줍니다.',
    caution: '여유를 당연시해 저축·자기계발 같은 노력을 소홀히 할 수 있습니다.',
    action: '"여유의 돈"과 "미래 투자의 돈"을 비율로 나눠 지출 계획을 세워 두시기 바랍니다.',
  ),
  'PP-FT-DENTED': _FindingTemplate(
    ...
    caution: '여유가 없으니 짜증·예민함이 관계에 자주 번집니다.',
    scenario: '야근이 이어지거나 양가 행사가 몰린 시기에, 두 분 다 지친 상태로 귀가해 사소한 것에도 폭발하는 날이 늘어납니다.',
    action: '일주일에 하루는 반드시 아무것도 하지 않는 "회복일"을 정해 두시기 바랍니다.',
  ),
  'PP-IL-SANGEN-LOW': _FindingTemplate(
    ...
    action: '40대부터는 매년 종합검진을 같이 받고, 보험·의료비 계획을 공동 자산처럼 관리하시기 바랍니다.',
  ),
  'PP-IL-SANGEN-HIGH': _FindingTemplate(
    ...
    action: '기본 건강검진과 운동 루틴은 자신감 여부와 상관없이 일정에 고정해 두시기 바랍니다.',
  ),
};
...
const Map<YinYangPatternKind, _FindingTemplate> _yinyangTemplate = {
  YinYangPatternKind.yangYinIdeal: _FindingTemplate(
    title: '음양의 이상적 조합',
    domain: '성향·대화',
    meaning: '한 분은 추진하는 양의 성향, 다른 분은 받아 주는 음의 성향이라는 뜻입니다. 전통 관상학이 가장 안정적이라 보는 짝입니다.',
    ...
    action: '반기에 한 번 정도 "역할을 바꿔서" 생활해 보는 실험을 해 보시기 바랍니다.',
  ),
  YinYangPatternKind.yangYang: _FindingTemplate(
    ...
    action: '영역별 결정권자를 문서화하고, 갈등 시 3-3-3 규칙(3분 말하고 3분 듣고 3분 쉬기)을 적용하시기 바랍니다.',
  ),
  YinYangPatternKind.yinYin: _FindingTemplate(
    ...
    action: '격주 단위로 "이번 주 결정 담당" 을 정해 두고, 그 주는 그 사람이 무조건 리드하는 규칙을 만드시기 바랍니다.',
  ),
  YinYangPatternKind.balancedBoth: _FindingTemplate(
    ...
    action: '분기에 한 번씩 "약간 모험적인 활동" 을 일부러 기획해 보시기 바랍니다 — 여행지 하나, 새 취미 하나.',
  ),
  YinYangPatternKind.oneBalanced: _FindingTemplate(
    ...
    action: '중심 쪽도 주기적으로 감정 환기가 필요합니다 — 혼자 시간, 친구와의 시간을 주저 없이 가질 수 있게 지원해 주시기 바랍니다.',
  ),
  YinYangPatternKind.modernCross: _FindingTemplate(
    ...
    caution: '양가 부모·주변 사람과 기대 충돌이 생기기 쉽습니다.',
    scenario: '양가 행사·육아·경제 활동 분담 같은 이슈에서, 양쪽 부모의 전통적 기대와 부딪혀 스트레스가 관계로 튀는 날이 반복됩니다.',
    action: '양가에 "우리는 이렇게 살겠다" 는 역할 분담을 일찍 분명히 선언해 두시기 바랍니다.',
  ),
};
...
const Map<String, _FindingTemplate> _zoneTemplate = {
  'ZP-UP-BOTH-STRONG': _FindingTemplate(
    ...
    action: '지적 대화만큼 "같이 밥 짓기·청소하기" 같은 일상 돌봄 루틴도 의식적으로 만들어 두시기 바랍니다.',
  ),
  'ZP-UP-BOTH-WEAK': _FindingTemplate(
    ...
    action: '연초마다 "3년 계획 문서" 한 장을 같이 작성해 두시기 바랍니다.',
  ),
  'ZP-MID-BOTH-STRONG': _FindingTemplate(
    ...
    action: '영역별 결정권을 미리 나누어 문서화하시기 바랍니다 — "주거는 A, 투자는 B" 식으로.',
  ),
  'ZP-MID-ONE-STRONG': _FindingTemplate(
    ...
    action: '분기마다 "부담이 쏠린 쪽" 이 누구인지 서로 체크하고, 작업을 재분배하시기 바랍니다.',
  ),
  'ZP-LO-BOTH-STRONG': _FindingTemplate(
    ...
    action: '가족 시간과 부부 시간을 달력에서 분리해 기록해 두시기 바랍니다.',
  ),
  'ZP-LO-BOTH-WEAK': _FindingTemplate(
    ...
    action: '하루 한 번 직설적 애정 표현 ("사랑해", "고마워") 을 루틴으로 고정하시기 바랍니다.',
  ),
  'ZP-XC-UP-DOWN': _FindingTemplate(
    ...
    action: '서로의 강점을 명확히 언어화하고, 상대 역할을 비웃지 않기로 합의해 두시기 바랍니다.',
  ),
  'ZP-MIRROR-ALL': _FindingTemplate(
    ...
    action: '둘 다 안 해 본 활동(새로운 운동·취미·여행지)을 분기마다 하나씩 시도해 보시기 바랍니다.',
  ),
  'ZP-COMPLEMENT-ALL': _FindingTemplate(
    ...
    action: '초반 갈등 기간을 "적응 단계" 로 명시적으로 받아들이고, 결혼·동거 결정은 서두르지 마시기 바랍니다.',
  ),
};
...
class CompatFinding {
  ...
  factory CompatFinding.fromOrgan(OrganPairEvidence e) {
    ...
    return CompatFinding(
      ...
      action: positive
          ? '$partKo 결이 잘 맞는다는 점을 서로 자주 확인해 주시면 관계가 더 탄탄해집니다.'
          : '감정이 상하는 상황에서는 바로 반응하지 말고 10분 뒤에 다시 이야기하는 규칙을 두시기 바랍니다.',
    );
  }
  ...
  factory CompatFinding.fromPalace(PalacePairEvidence e) {
    ...
    return CompatFinding(
      ...
      caution: positive
          ? '비슷한 강점끼리 만나 자만하기 쉬우니 기본기를 놓치지 마십시오.'
          : '${e.palace.domainKo} 문제가 터지면 두 분 모두 힘들어지니 미리 대비가 필요합니다.',
      scenario: positive
          ? null
          : '${e.palace.domainKo} 쪽에서 문제가 생기면 한쪽이 다른 쪽을 받쳐 주기 어려워 이슈가 길게 끌릴 수 있습니다.',
      action: positive
          ? '${e.palace.domainKo}에서 오는 이점을 당연하게 여기지 말고, 서로에게 말로 자주 인정해 주시기 바랍니다.'
          : '${e.palace.domainKo} 영역에 미리 룰을 정해 두고, 한쪽이 흔들릴 때 다른 쪽이 지원하는 구조를 짜 두십시오.',
    );
  }
